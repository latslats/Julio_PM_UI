...
import {
  ...
  size,
  useFloating
} from "./chunk-DKB5XR5L.js";
import {
  Combination_default,
  hideOthers
} from "./chunk-I72EI7OJ.js";
import {
  require_react_dom
...
function useSize(element) {
  const [size2, setSize] = React21.useState(void 0);
  useLayoutEffect22(() => {
    if (element) {
...
var useInsertionEffect2 = React28[" useInsertionEffect ".trim().toString()] || useLayoutEffect22;
...
var RovingFocusGroup = React30.forwardRef(
  (props, forwardedRef) => {
...
var RovingFocusGroupImpl = React30.forwardRef((props, forwardedRef) => {
  const {
  ...
  const [isTabbingBackOut, setIsTabbingBackOut] = React30.useState(false);
  const handleEntryFocus = useCallbackRef(onEntryFocus);
  const getItems = useCollection(__scopeRovingFocusGroup);
  const isClickFocusRef = React30.useRef(false);
  const [focusableItemsCount, setFocusableItemsCount] = React30.useState(0);
  React30.useEffect(() => {
    const node = ref.current;
    if (node) {
  ...
  return (0, import_jsx_runtime15.jsx)(
    RovingFocusProvider,
    {
      ...
      loop,
      currentTabStopId,
      onItemFocus: React30.useCallback(
        (tabStopId) => setCurrentTabStopId(tabStopId),
        [setCurrentTabStopId]
      ),
      onItemShiftTab: React30.useCallback(() => setIsTabbingBackOut(true), []),
      onFocusableItemAdd: React30.useCallback(
        () => setFocusableItemsCount((prevCount) => prevCount + 1),
        []
      ),
      onFocusableItemRemove: React30.useCallback(
        () => setFocusableItemsCount((prevCount) => prevCount - 1),
        []
      ),
...
var RovingFocusGroupItem = React30.forwardRef(
  (props, forwardedRef) => {
    const {
      __scopeRovingFocusGroup,
      focusable = true,
...
var useReactId2 = React32[" useId ".trim().toString()] || (() => void 0);
var count3 = 0;
function useId2(deterministicId) {
  const [id, setId] = React32.useState(useReactId2());
  useLayoutEffect2(() => {
    if (!deterministicId)
